...
public class Employee extends User {
	...
	EmployeeDecorator EEE = new EmployeeDecorator(this.userId);
	...
	public void updateEmployee(String name, int phone, double salary) {
		String query = "UPDATE `employee` SET `employeeName`='"+name+"', `phoneNumber`='+972"+phone+"', `role`='General', `salary`="+salary+" WHERE `userId`='"+this.userId+"';";
        try {
        	Database db = Database.getInstance();
        	db.DBupdate(query);
