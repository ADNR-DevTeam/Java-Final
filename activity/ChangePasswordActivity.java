...
public class ChangePasswordActivity extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource().equals(buttonSubmit)) {
			if (oldPF.getText().equals(newPF.getText()))
			{
				JOptionPane.showMessageDialog(this,"The new password is the same as the old password. Try again!"); 
			}
			else if (newPF.getText().length() < 4 )
			{
				JOptionPane.showMessageDialog(this,"Password must be longer then 3 characters. Try again!");
			}
			else { 
